              ...
              $.getJSON( "coin_live_price/"+$filter+".json", function( json ) {
                ...
                //    console.log($dPrice);
                    ...
                    console.log('Last:'+$lastDate);
                      ...
                      console.log($currentMatchDate + ' - ' + $dPrice);
                      if($filter == 'XRP' || $filter == 'ETC' || $filter == 'ETH'){
  ...
  /*
            socket.on('trades', function (tradeMsg) {
                ...
                if($filter == $coin){
                    ...
                    $currentDate = $cDate.currrentMatchDate;
                    $currentMatchDate = $cDate.currentDate;
                    $beforePrice = parseFloat($('#current_price').val());
                    if($filter =='XRP'){
                        $dPrice = $price;
                    }else{
    ...
    function addRect(){
      textElm = document.getElementsByClassName('amcharts-guide-live-guide');
      parent = textElm[2].parentNode;
      $transRaw = $('.amcharts-guide-live-guide:eq(2)').attr('transform');
      $transSplit = $transRaw.split(',');
      $transSplit_1 = $transSplit[0];
      $transSplit_2 = $transSplit[1];

      $finalX = $transSplit_1.match(/\d/g).join("");
      $finalY = $transSplit_2.match(/\d/g).join("");
